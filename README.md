...
With a token a link resolves to a track, and a playable track's `/streams` gives stream endpoints on the API that need the token - so a media element cannot fetch them - and redirect to a signed HLS playlist on SoundCloud's CDN, which allows cross-origin reads.
The page fetches the playlist and its MP3 segments, six at a time, joins them and decodes the whole track through the same path as a dropped file, so the timeline, seeking and looping all work; tracks over twenty-five minutes are refused before download, since an hour of decoded stereo is over a gigabyte, and a long mix is better captured from a tab.
Only tracks whose uploader allows off-platform streaming - `access: playable`, which is most SoundCloud-native artists - play in full; label releases come back as `preview`, thirty seconds at most outside SoundCloud's own player, and the panel offers to open the track there and capture that tab instead.
The HUD credits the uploader with a link to the track on SoundCloud, as the terms ask of a custom player.
