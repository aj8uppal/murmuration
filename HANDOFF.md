...
| `mode` | Render mode. `0` runs the particle simulation; `1` runs the voyage flight, which skips the flow, sim and backdrop passes and draws the light field instead; `2` runs the current, the strand sculpture flown through along the flight path; `3` the plate, which draws its own scene from `modeData` and skips the field; `4` the warp, the tunnel flown through, drawn from `warp` as additive light with no depth. | `0..4` | `0`. |
| `warp` | The warp's uniforms, twenty-eight floats filled by `#updateWarp` in `main.js`: distance travelled (wrapped), speed, boost and spin; the curvature vector of the arc ahead and the gaze; the bank, the flow phase, and the thread and star counts (which the renderer writes from the two fields below); two waves, each a position in travel units and an amplitude; the flash, the onset's kick, the energy and the warmth; the vanishing point and the shake offset, both in uv; the distance travelled unwrapped, for the haze. The layout is documented on the `warp*` fields in `common.wgsl`. | `Float32Array(28)` | zeros. |
| `warpStars` `warpRails` | Instance counts for the warp's star field and its threads; the quality presets supply the stars and the threads are twelve. `0` draws none. | stars: see `main.js` presets; rails: `0` or `12` | `0`. |
