...
| `warp` | The warp's uniforms, twenty-four floats filled by `#updateWarp` in `main.js`: distance travelled, speed, boost and spin; the curvature vector of the arc ahead and the gaze; the bank, the flow phase, and the rail and star counts (which the renderer writes from the two fields below); two pulses, each a position in travel units and an amplitude; the flash, the beat kick, the energy and the warmth; the vanishing point and the shake offset, both in uv. The layout is documented on the `warp*` fields in `common.wgsl`. | `Float32Array(24)` | zeros. |
| `warpStars` `warpRails` | Instance counts for the warp's star field and its rails; the quality presets supply the stars and the rails are eight. `0` draws none. | stars: see `main.js` presets; rails: `0` or `8` | `0`. |
